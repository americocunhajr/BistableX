...
for count = [7:10] %60:93
    if count<10
        name = ['mono_left_symmetric/DPsv0000',num2str(count),'.mat'];
    elseif count>=10&count<100
        name = ['mono_left_symmetric/DPsv000',num2str(count),'.mat'];
    elseif count>=100
        name = ['mono_left_symmetric/DPsv00',num2str(count),'.mat'];
    end
    load(name)
    ...
    if count == 7
        ...
        velo = smooth(X3(:,2));
        v_vec_l = [v_vec_l; velo];
        acc  = [acc; X1(:,2)];
        volt_l = [volt_l; X2(:,2)];
    end  
end
dt = X3(2,1);
ss_velo_l = v_vec_l/10;
disp = Integration_frequency(v_vec_l,dt);
ss_disp_l = detrend(disp)./(1/dt)/10000;
...
[disp_poinc, velo_poinc, disp_interp, velo_interp, time_interp] = poincare_estimation(time_vector_sym_magL,ss_disp_l,ss_velo_l,6.5,32768);

disp_l = disp_interp(end*0.1:65536/2560:end*0.8);
velo_l = velo_interp(end*0.1:65536/2560:end*0.8);
...
for count = [7:10] %60:93
    if count<10
        name = ['mono_right_symmetric/DPsv0000',num2str(count),'.mat'];
    elseif count>=10&count<100
        name = ['mono_right_symmetric/DPsv000',num2str(count),'.mat'];
    elseif count>=100
        name = ['mono_right_symmetric/DPsv00',num2str(count),'.mat'];
    end
    load(name)
    ...
    if count == 7
        ...
        velo = smooth(X3(:,2));
        v_vec_r = [v_vec_r; velo];
        acc  = [acc; X1(:,2)];
        volt_r = [volt_r; X2(:,2)];
    end  
end
dt = X3(2,1);
ss_velo_r = v_vec_r/10;
disp = Integration_frequency(v_vec_r,dt);
ss_disp_r = detrend(disp)./(1/dt)/10000;
...
disp_r = disp_interp(end*0.1:65536/2560:end*0.8);
velo_r = velo_interp(end*0.1:65536/2560:end*0.8);
...
%%
save('data_poincare_monostable_sym','disp_l',"velo_l",'disp_map_l',"velo_map_l",'disp_r',"velo_r",'disp_map_r',"velo_map_r",'volt_r','volt_l','time_vector_sym_magR')